...
from auth_system import get_active_version_id, log_audit, save_reconciliation_data
...
def interfund_bank_reconciliation_app(all_bank_dfs: dict):
    ...
    if 'unmatched_interfund_df' in st.session_state:
        ...
        # Add to interfund_bank_reconciliation_page.py - at the end of function

        # Auto-save results
        if 'authenticated' in st.session_state and st.session_state['authenticated']:
            if any([
                not st.session_state.get('matched_interfund_df', pd.DataFrame()).empty,
                not st.session_state.get('unmatched_interfund_df', pd.DataFrame()).empty
            ]):
                ...
                if version_id:
                    ...
                    for df_name in ['matched_interfund_df', 'unmatched_interfund_df',
                                'unmatched_bank_interfund']:
                        df = st.session_state.get(df_name, pd.DataFrame())
                        if not df.empty:
                            save_reconciliation_data(version_id, df_name, df, current_date, user_id)
                    
                    log_audit(user_id, 'INTERFUND_RESULTS_SAVED', 
                            f'Saved interfund reconciliation results')
